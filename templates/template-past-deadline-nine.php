<?php
/**
 * Template Name: Past Deadline 9 Months
 *
 * @package Avada
...
<p style="margin-top:10px;font-size:16px;" class="link-for-past">
    <a   href="#sixmonths"  style="color:black !important;margin-left:0px !important;"  class="activeee">   Past Deadline  (After 9  Months )    </a>  |
...
<p style="font-size:23px;margin-top:60px;"> 1 - Institutions Past Deadline (After 9 Months) </p>
<hr>
<?php 
    ...
    foreach ($institution_query->posts as $institution_id) { 
        ...
        if (have_rows('admission_deadlines', $institution_id)) {
            while (have_rows('admission_deadlines', $institution_id)) {
                ...
                if ($deadline_timestamp && $deadline_timestamp <= $nineToTwelveMonthsAgoEnd && $deadline_timestamp > $nineToTwelveMonthsAgoStart) {
            ...
            // Only add institutions with the relevant deadlines to our results.
...
foreach ($grouped_institutions as $country => $institutions) {
    ...
    echo "<tr> <th> University Name </th> <th> Label </th>  <th> Past Deadline <br> (After 9 Months) </th>  </tr>";
