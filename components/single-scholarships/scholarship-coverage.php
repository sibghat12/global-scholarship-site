...
<section id="scholarship-coverage" class="gs-scholarship-coverage">
    <h2><?php echo  $scholarship_title; ?> Coverage</h2>
    <?php 
        if(count($coverage) == 1 ) : ?>
            <p>As a <?php echo get_adjective_scholarship_amount($scholarship_type); ?>
            scholarship for international students, this offer covers <?php echo $coverage[0]['coverage']; ?></p>
            
        <?php elseif(count($coverage) > 1 ) : ?>
            <p>As a fully-funded scholarship for international students, it covers the following:</p>
            <ul class="fa-ul">
